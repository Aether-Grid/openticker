...
interface ParamRow {
  ...
  key: string
  type: ValueType
  /** The current value, narrowed by `type`. */
...
/** Coerces a raw param value into the row's displayable value for its type. */
function coerce(value: unknown, type: ValueType): string | number | boolean {
  if (type === 'number') return typeof value === 'number' ? value : 0
  if (type === 'boolean') return typeof value === 'boolean' ? value : false
  // Strings keep their text; non-string scalars are stringified, objects/arrays
  // are JSON-encoded so they round-trip readably rather than showing [object].
...
const ROW_TYPE_ITEMS = [
  { label: 'text', value: 'string' as ValueType },
  { label: 'number', value: 'number' as ValueType },
  { label: 'boolean', value: 'boolean' as ValueType }
]
...
/** Switches a row's value type, coercing the current value into the new type. */
function setType(index: number, type: ValueType) {
  const row = rows.value[index]
      ...
      <div class="space-y-2">
        ...
        >
          ...
          <USelect
            ...
            class="font-data w-full"
            @update:model-value="(v: ValueType) => setType(index, v)"
          />
